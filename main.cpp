...
int main() {
	...
	Spell easy_snailsville_enemy1_spell1("Cracked Hammer", "The user shapes a jagged, darkened hammer from twisted metal and strikes the target's skull with a resonant clang.", 4, 3);
...
//Snailsville | Easy Enemy no.2 | Shellcarver Apprentice
	...
	Spell easy_snailsville_enemy2_spell1("Shattered Rock", "The user hurls a jagged fragment of stone, cracking it against the target with a sharp impact.", 5, 3);
	ChainingSpell easy_snailsville_enemy2_spell2("Pebble Barrage", "A swarm of sharp, unnatural pebbles erupts from the user's hands, cutting at the target relentlessly.", 0.75, 4, 3, 8);
...
//Snailsville | Easy Enemy no.3 | Glimmering Courier
	...
	Spell easy_snailsville_enemy3_spell1("Blinding Flash", "The user emits a sudden, piercing light that sears the target's eyes, leaving a lingering sting.", 3, 2);
...
//Snailsville | Easy Enemy no.4 | Herbalist Snail
	...
	Spell easy_snailsville_enemy4_spell1("Petal Maelstrom", "The user summons a spiraling storm of brittle, blackened petals that slice at anything in its path.", 4, 3);
	Spell easy_snailsville_enemy4_spell2("Thorn Lash", "The user lashes thorned vines at the target, tearing through flesh and armor alike.", 3, 2);
...
//Snailsville | Easy Enemy no.5 | Lampbearer Snail
	...
	Spell easy_snailsville_enemy5_spell1("Searing Gleam", "The user unleashes a piercing flash of corrupted light, scorching eyes and leaving a lingering daze.", 4, 3);
	ChainingSpell easy_snailsville_enemy5_spell2("Waxstorm", "The user trembles violently, raining molten wax shards that stick and burn, scalding flesh and armor alike.", 0.5, 4, 3, 10);
...
//Snailsville | Easy Enemy no.6 | Tavern Worker Snail
	...
	Spell easy_snailsville_enemy6_spell1("Barrel Smash", "The snail conjures a warped, oozing barrel and hurls it at full speed, splintering whatever it hits.", 6, 4);
	Spell easy_snailsville_enemy6_spell2("Piercing Brew", "The snail slaps its hands together, forming a condensed stream of fermented sludge that pierces the target.", 4, 3);
...
//Snailsville | Easy Enemy no.7 | Fisher Snail
	...
	Spell easy_snailsville_enemy7_spell1("Mire Bubble", "The snail exhales a swollen, murky bubble of mucus that drifts forward before bursting violently against the target.", 4, 3);
...
//Snailsville | Easy Enemy no.8 | Mud Leech
	...
	LifeStealSpell easy_snailsville_enemy8_spell1("Sanguine Leech", "A writhing, vein-thick tendril lashes out and drains vitality straight from the victim's veins.", 6, 4, 60);
	Spell easy_snailsville_enemy8_spell2("Grave Mud Slam", "A hulking mass of grave-cold mud forms into a hand and crashes down upon the target.", 4, 3);
...
//Snailsville | Easy Enemy no.9 | Tiny Rotworm
	...
	LifeStealSpell easy_snailsville_enemy9_spell1("Frail Siphon", "A thin tendril pierces the flesh and steals a whisper of vitality.", 4, 3, 50);
...
//Snailsville | Medium Enemy no.1 | Ironshell Knight
	...
	Spell medium_snailsville_enemy1_spell1("Ferric Impact", "The knight encases his gauntlet in hardened iron and delivers a crushing blow.", 6, 3);
...
//Snailsville | Medium Enemy no.2 | Shadow Sentry
	...
	Spell medium_snailsville_enemy2_spell2("Gloomblade", "A blade of hardened darkness forms and cleaves through the target.", 7, 3);
	ChainingSpell medium_snailsville_enemy2_spell3("Eclipse Barrage", "Weapons fall aside as the sentry's fists become void-shrouded, striking in relentless succession.", 1, 3, 1, 5);
...
//Snailsville | Medium Enemy no.3 | Leech Scout
	...
	LifeStealSpell medium_snailsville_enemy3_spell1("Crimson Siphon Strike", "A brutal tendril-infused blow that tears vitality straight from the veins.", 7, 4, 70);
...
//Snailsville | Medium Enemy no.4 | Obsidian Shell Snail
	...
	Spell medium_snailsville_enemy4_spell1("Shadowfist Strike", "The user envelops their claw in living shadow and slams it into the foe, chilling the soul", 6, 3);
...
//Snailsville | Medium Enemy no.5 | Venomspire Snail
	...
	StatusEffectSpell medium_snailsville_enemy5_spell1("Venomous Fang Strike", "A fang coated in a writhing black venom tears into the target, burning flesh and soul alike", 6, 4, 70, &medium_snailsville_enemy5_status1);
	HpStatusEffect medium_snailsville_enemy5_status2("Blight Poison", 3, 1, -0.75);
	StatusEffectSpell medium_snailsville_enemy5_spell2("Blighted Jab", "A quick strike that injects a creeping poison, leaving the target weakened and shivering", 4, 3, 70, &medium_snailsville_enemy5_status2);
...
//Snailsville | Medium Enemy no.6 | Creeping Mollusk
	...
	Spell medium_snailsville_enemy6_spell1("Umbral Strike", "The user emerges from creeping darkness and delivers a sudden, silent blow", 6, 3);
	Spell medium_snailsville_enemy6_spell2("Shadowfall Kick", "The user bursts from the shadows and crashes into the target with a brutal kick", 8, 4);
...
//Snailsville | Medium Enemy no.7 | Mudspike Leech
	...
	ChainingSpell medium_snailsville_enemy7_spell1("Whirling Abyss Dance", "The user twists into a violent spinning assault, striking the target again and again with relentless force", 0.5, 3, 2, 10);
	LifeStealSpell medium_snailsville_enemy7_spell2("Thorn of the Bloodleecher", "The user launches a cursed thorn that burrows into the target and siphons their life essence", 6, 4, 70);
	ChainingSpell medium_snailsville_enemy7_spell3("Storm of Mire Spikes", "The user hurls a barrage of jagged mudspikes that rip through flesh like shards of stone", 0.75, 3, 2, 8);
...
//Snailsville | Hard Enemy no.1 | Bloodcarapace Marauder
	...
	Spell hard_snailsville_enemy1_spell1("Iron Cataclysm Fist", "The marauder encases his arm in jagged metal and delivers a devastating strike.", 12, 4);
	HpStatusEffect hard_snailsville_enemy1_status1("Hemorrhage", 4, 2, -1.5);
	StatusEffectSpell hard_snailsville_enemy1_spell2("Carapace Rend", "Hardened claws tear deep, leaving a brutal bleeding wound.", 6, 2, 70, &hard_snailsville_enemy1_status1);
	LifeStealSpell hard_snailsville_enemy1_spell3("Crimson Devourer", "A massive tendril lashes out and drains vitality without mercy.", 8, 3, 80);
...
//Snailsville | Hard Enemy no.2 | Soulbound Enforcer
	...
	ChainingSpell hard_snailsville_enemy2_spell2("Eternal Soulstorm", "Shattered fragments of fallen souls erupt in a relentless barrage.", 1, 4, 4, 12);
...
//Snailsville | Hard Enemy no.3 | Venomspike Assassin
	...
	StatusEffectSpell hard_snailsville_enemy3_spell1("Venomspike Shot", "The user launches a razor-sharp venomspike infused with deadly toxin", 10, 3, 70, &hard_snailsville_enemy3_status1);
	Spell hard_snailsville_enemy3_spell2("Umbral Jab", "The user melts into shadow, reappearing behind the target to drive a precise strike into their side", 7, 2);
	Spell hard_snailsville_enemy3_spell3("Umbral Kick", "The user emerges from the darkness and lands a brutal kick before fading back into shadow", 9, 3);
...
//Snailsville | Hard Enemy no.4 | Deathcoil Snail
	...
	Spell hard_snailsville_enemy4_spell1("Coiling Charge", "The snail coils into a spinning mass and violently slams into the target", 11, 3);
	ChainingSpell hard_snailsville_enemy4_spell2("Thunderous Spiral", "The creature spins uncontrollably, crashing into the target again and again like a living battering ram", 1.5, 3, 1, 5);
	Spell hard_snailsville_enemy4_spell3("Dust Vortex", "The snail spins at terrifying speed, unleashing a choking storm of dust and debris", 7, 2);
...
//Snailsville | Hard Enemy no.5 | Obsidian Champion
	...
	Spell hard_snailsville_enemy5_spell1("Obsidian Fist", "The champion cloaks its fist in jagged obsidian and strikes with crushing force", 13, 4);
	ChainingSpell hard_snailsville_enemy5_spell2("Shards of Obsidian", "The champion hurls razor-sharp obsidian shards in a deadly scatter towards the target", 1, 3, 1, 10);
...
// Snailsville | Hard Enemy no.6 | Tidal Leech Commander
	...
	LifeStealSpell hard_snailsville_enemy6_spell1("Hydro Drain", "The commander splashes corrosive water, masking tendrils that sap vitality from the target", 12, 4, 75);
	HpStatusEffect hard_snailsville_enemy6_status1("Hemorrhage", 1, 4, -1.5);
	StatusEffectSpell hard_snailsville_enemy6_spell2("Water Spike", "A jet of water pierces the target, causing bleeding", 7, 3, 70, &hard_snailsville_enemy6_status1);
	ChainingSpell hard_snailsville_enemy6_spell3("Crimson Bubble Barrage", "The commander fires a relentless stream of blood-filled bubbles at the target", 0.5, 3, 1, 16);
...
//Snailsville | Boss no.1 | Claris, the Shell Empress
	...
	StatusEffectSpell snailsville_boss1_spell1("Ooze Lash", "The user lashes a writhing mass of blackened slime at the target, corroding flesh and spirit.", 12, 3, 70, &snailsville_boss1_status1);
	Spell snailsville_boss1_spell2("Carapace Slam", "The user hurls their jagged shell at the target, crushing with brutal force.", 9, 2);
	StatusEffectSpell snailsville_boss1_spell3("Putrescent Cloud", "The user exhales a toxic cloud towards the target, sapping vitality.", 5, 2, 70, &snailsville_boss1_status1);
	Spell snailsville_boss1_spell4("Spinning Devastation", "The user spins in their shell at full speed, battering the target mercilessly.", 15, 5);
...
//Snailsville | Boss no.2 | Mathan, the Bloodcarapace
	...
	LifeStealSpell snailsville_boss2_spell1("Sanguine Sap", "The user rends the target's flesh with a cursed cut and siphons their life force.", 8, 3, 65);
	LifeStealSpell snailsville_boss2_spell2("Crimson Leeching", "The user bites deep, draining the vitality of the target through dark blood magic.", 10, 3, 75);
	Spell snailsville_boss2_spell3("Piercing Hemorrhage", "The user slams their hands together, unleashing a pressurized jet of coagulated blood at the target.", 11, 2);
	HpStatusEffect snailsville_boss2_status1("Hemorrhaging", 6, 3, 2.5);
	StatusEffectSpell snailsville_boss2_spell4("Bloody Web", "The user conjures a crimson web of congealed blood, slicing through the target.", 5, 3, 70, &snailsville_boss2_status1);
...
//Snailsville | Boss no.3 | Andreas, the Tidal Snail
	...
	Spell snailsville_boss3_spell1("Tidal Crush", "The user summons a towering wave that smashes down upon the target.", 16, 5);
	ChainingSpell snailsville_boss3_spell2("Relentless Drizzle", "The user conjures a stormcloud, raining dagger-like drops upon the target", 0.5, 3, 6, 18);
	HpStatusEffect snailsville_boss3_status1("Drowning", 5, 1, 4);
	StatusEffectSpell snailsville_boss3_spell3("Choking Bubble", "The user traps the target in a suffocating bubble", 5, 3, 65, &snailsville_boss3_status1);
	Spell snailsville_boss3_spell4("Maelstrom Spin", "The user creates a violent whirlpool that drags the target in", 9, 3);
...
//Snailsville | Boss no.4 | Peklorex, the Infernal Slug
	...
	Spell snailsville_boss4_spell1("Searing Fist", "The user heats up their fist and crushes the target with molten force", 14, 4);
	HpStatusEffect snailsville_boss4_status1("Cursed Flames", 4, 1, 4);
	StatusEffectSpell snailsville_boss4_spell2("Infernal Breath", "The user exhales a scorching inferno upon the target", 7, 4, 60, &snailsville_boss4_status1);
	...
	ChainingSpell snailsville_boss4_spell4("Dance of Damnation", "The user writhes possessed by dark flames, striking with ferocity", 2, 3, 2, 5);
	...
	while (true) {
		...
		saveGame(currentMap->getPlayerPosition(), player, slot);
